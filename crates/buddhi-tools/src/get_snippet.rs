...
impl Tool for GetSnippetTool {
    ...
    async fn execute(&self, args: serde_json::Value, project_root: &Path) -> Result<ToolResult> {
        ...
        let start = args
            .get("start")
            .and_then(|s| s.as_u64())
            .ok_or_else(|| BuddhiError::ToolExecution("Missing start argument".to_string()))?
            as usize;
        let end = args
            .get("end")
            .and_then(|e| e.as_u64())
            .ok_or_else(|| BuddhiError::ToolExecution("Missing end argument".to_string()))?
            as usize;

        let safe_path = PathGuard::validate(path, project_root)?;
        let content =
            fs::read_to_string(safe_path).map_err(|e| BuddhiError::ToolExecution(e.to_string()))?;
        let lines: Vec<&str> = content.lines().collect();

        if start >= end || start >= lines.len() {
            return Err(BuddhiError::ToolExecution("Invalid line range".to_string()));
        }
