...
impl AgentLoop {
    ...
    fn mock_llm_call(&self, attempt: usize, _task: &str) -> Option<ToolCall> {
        if attempt == 0 {
            // First attempt: write broken code
            Some(ToolCall {
                id: "call_1".to_string(),
                name: "write_file".to_string(),
                arguments: json!({
                    "path": "src/test_agent.rs",
                    "content": "pub fn broken_code() { let x: i32 = \"string\"; }"
                }),
            })
        } else if attempt == 1 {
            // Second attempt: write working code
            Some(ToolCall {
                id: "call_2".to_string(),
                name: "write_file".to_string(),
                arguments: json!({
                    "path": "src/test_agent.rs",
                    "content": "pub fn working_code() { let x: i32 = 42; }"
                }),
            })
        } else {
            // Finish
